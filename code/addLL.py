	...
	while(year<=year_end):
		...
		for i in range(10):
			if type == 'gaModel':
				...
				modelL=etasGa.loadModelFromFile('../Zona3/scModel/listgamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txt')	
			modelL = etasGa.limitTo12(modelL)
			modelO=model.loadModelFromFile('../Zona2/realData/'+region+'real'+"_"+str(year)+'.txt')
			loglikeValue=csep.loglikelihood.calcLogLikelihood(modelL,modelO)
			modelL.loglikelihood=0
			modelL.mag=True
			if type == 'gaModel':
				etasGa.saveModelToFile(modelL,'../Zona3/scModel/gamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txt')	
			elif region == 'EastJapan':
				etasGa.saveModelToFile(modelL,'../Zona3/scModel/eastgamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txt')	
			else:
				etasGa.saveModelToFile(modelL,'../Zona3/scModel/listgamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txt')
			...
			if type == 'gaModel':
				with open('../Zona3/scModel/gamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txtloglikelihood.txt', "w") as myfile:
					myfile.write(str(loglikeValue))
					myfile.write("\n")
			elif region == 'EastJapan':
				with open('../Zona3/scModel/eastgamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txtloglikelihood.txt', "w") as myfile:
					myfile.write(str(loglikeValue))
					myfile.write("\n")
			else:
				etasGa.saveModelToFile(modelL,'../Zona3/scModel/listgamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txt')
				with open('../Zona3/scModel/listgamodel'+region+'_'+str(depth)+'_'+str(year)+str(i)+'.txtloglikelihood.txt', "w") as myfile:
					myfile.write(str(loglikeValue))
					myfile.write("\n")
		year+=1
