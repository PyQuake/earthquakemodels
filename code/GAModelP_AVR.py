...
def evaluationFunction(individual, modelOmega):
    ...
    for i in range(len(modelOmega)):
        ...
        if hasattr(modelOmega, 'values4poisson'):
            modelLambda.bins = calcNumberBins(
                modelLambda.bins,
                modelOmega[i].bins,
                modelOmega.values4poisson)
        else:
            modelLambda.bins = calcNumberBins(
                modelLambda.bins, modelOmega[i].bins)
...
def gaModel(type_m, NGEN, CXPB, MUTPB, modelOmega,
            year, region, depth, n_aval=500):
    y = int(n_aval / NGEN)
    x = n_aval - y * NGEN
    n = x + y
