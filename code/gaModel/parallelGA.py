...
def gaModel(NGEN,
    ...
    It uses two parallel system: 1, simple, that splits the ga evolution
    between cores and 2, that distributes the islands
    """
    start = time.clock()
    # Attribute generator
    ...
    toolbox.register("individual", tools.initRepeat, creator.Individual,
                     toolbox.attr_float, len(modelOmega[0].bins))
    ...
    # calculating the number of individuals of the populations
    ...
    pop = toolbox.population(int(n / size))
    ...
    fitnesses = list(map(toolbox.evaluate, pop))  # need to pass 2 model.bins.
    # One is the real data, the other de generated model
    ...
    for g in range(NGEN):
        ...
        # but the last island[rank] best_pop
        # Elitism
        ...
        if g % (FREQ - 1) == 0 and g > 0:
            ...
            comm.sendrecv(sendobj=best_inds, dest=dest, source=origin)
            # rotation
            target += 1
            origin = (rank - (target + 1)) % size
            dest = (rank + ((target + 1) + size)) % size
