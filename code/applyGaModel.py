...
def execParallelGARandomParSC(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    means = list()
    ...
    for i in range(times):
        ...
        modelo.mag = True
        if save == True:
            etasGa.saveModelToFile(modelo, 'sc-parallel-random/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
        with open('time_exec/sc-parallel-random'
                  + region + '_' + str(depth) + "_" + str(year + qntYears) + '.txt', "a") as myfile:
            myfile.write(str(end - start))
            myfile.write("\n")


def execParallelListGARandomParSC(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    means = list()
    for i in range(qntYears):
    ...
    for i in range(times):
        ...
        if save == True:
            start = time.clock()
            etasGa.saveModelToFile(modelo, 'sc-parallelList-random/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
        with open('time_exec/sc-parallelList-random'
                  + region + '_' + str(depth) + "_" + str(year + qntYears) + '.txt', "a") as myfile:
            myfile.write(str(end - start))
            myfile.write("\n")


def execParallelListGARandomPar(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    means = list()
    for i in range(qntYears):
    ...
    for i in range(times):
        ...
        if save == True:
            start = time.clock()
            etasGa.saveModelToFile(modelo, 'parallelList-random/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
...
def execParallelGARandomPar(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    means = list()
    for i in range(qntYears):
        observation = model.loadModelFromFile('../Zona3/sc/3.0'
    ...
    for i in range(times):
        ...
        modelo.mag = True
        if save == True:
            etasGa.saveModelToFile(modelo, 'parallel-random/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
...
def execParallelGA(year, region, depth, qntYears=5, times=10, save=True):
    ...
    for i in range(times):
        ...
        modelo.mag = True
        if save == True:
            etasGa.saveModelToFile(modelo, 'parallel-fixcxmt/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
...
def execParallelListGA(year, region, depth, qntYears=5, times=10, save=True):
    ...
    for i in range(times):
        ...
        modelo.mag = True
        if save == True:
            etasGa.saveModelToFile(modelo, 'parallelList-fixcxmt/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
...
def execGaModel(year, region, depth, qntYears=5, times=10, save=True):
    ...
    for i in range(times):
        modelo = ga.gaModel(
            ...
            mean)
        if save == True:
            model.saveModelToFile(modelo, '../Zona3/gaModel/' +
                                  region +
                                  '_' +
                                  str(depth) +
                                  "_" +
...
def createRealModel(year, region, depth, withMag=True, save=False):
    ...
    if save == True:
        if observacao.mag == False:
            if depth == 0:
                model.saveModelToFile(observacao,
                                      '../Zona2/realData/' + region + 'real' + "_" + str(year) + '.txt', real=True)
            else:
                model.saveModelToFile(observacao,
                                      '../Zona2/realData/' + str(depth) + region + 'real' + str(depth) + "_" + str(year) + '.txt', real=True)
        else:
            model.saveModelToFile(observacao,
                                  '../Zona/' + region + 'realWithMag' + str(year) + '.txt', real=True)
...
def execGaModelClustered(year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    for i in range(qntYears):
        observation = model.loadModelFromFile('../Zona2/clustered/3.0'
                                              + region + 'real' + str(depth) + "_" + str(year + i) + '.txt')
        observation.bins = observation.bins.tolist()
        observations.append(observation)
    for i in range(times):
        modelo = ga.gaModel(
            ...
            depth)
        if save == True:
            model.saveModelToFile(modelo,
                                  '../Zona2/clustered_gaModel/' + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')


def execEtasGaModelClustered(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    for i in range(qntYears):
        observation = model.loadModelFromFile('../Zona2/clustered/3.0'
    ...
    for i in range(times):
        ...
        modelo.mag = True
        if save == True:
            etasGa.saveModelToFile(modelo, '../Zona2/clustered_listaGA_new/'
                                   + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')
...
def createRealModelClusteredII(year, region, depth, withMag=True, save=False):
    ...
    if save == True:
        if observacao.mag == False:
            model.saveModelToFile(observacao, '../Zona2/clusteredII/'
                                  + str(3.0) + region + 'real' + str(depth) + "_" + str(year) + '.txt', real=True)


def execGaModelClusteredII(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()
    for i in range(qntYears):
        observation = model.loadModelFromFile('../Zona2/clusteredII/3.0'
                                              + region + 'real' + str(depth) + "_" + str(year + i) + '.txt')
        observation.bins = observation.bins.tolist()
        observations.append(observation)
    for i in range(times):
        modelo = ga.gaModel(
            ...
            depth)
        if save == True:
            model.saveModelToFile(modelo, '../Zona2/clusteredII_gaModel/'
                                  + region + '_' + str(depth) + "_" + str(year + qntYears) + str(i) + '.txt')


def execEtasGaModelClusteredII(
        year, region, depth, qntYears=5, times=10, save=True):
    observations = list()

    for i in range(qntYears):
        observation = model.loadModelFromFile(
            '../Zona2/clusteredII/3.0' + region + 'real' + str(depth) + "_" + str(year + i) + '.txt')
        observation.bins = observation.bins.tolist()
        observations.append(observation)
...
def createandExecRealModelSCwithP_AVR(
    ...
    for i in range(qntYears):
        ...
        observacao = model.addFromCatalogP_AVR(
            observacao, catalogo, riskMap, year)
