...
def save2file(logbook, rank):
    ...
    f = open('parallel2/' + str(rank) + region + '_' +
             str(year) + '_' + str(depth) + '_logboo8-2.txt', "a")
    data = str(logbook)
    f.write(data)
    f.write('\n')
    f.close()
...
def gaModel(NGEN, n, modelOmega, year, region, depth, FREQ=10):
    ...
    toolbox.register("individual", tools.initRepeat, creator.Individual,
                     toolbox.attr_float, len(observations[0].bins))
    ...
    for region in regions:
        ...
        while ((year + qntYears) <= 2010):
            ...
            for i in range(times):
                GSIZE = 100
                POPSIZE = 500
                modelo = gaModel(GSIZE, POPSIZE, observations,
                                 year + qntYears, region, depth, FREQ=10)
                modelo.mag = True
                if save == True:
                    etasGa.saveModelToFile(modelo, './parallel2/' + region + '_' + str(depth) + "_" + str(
                        year + qntYears) + str(i) + '.txt', year=str(year + qntYears), type='a')
            year += 1
