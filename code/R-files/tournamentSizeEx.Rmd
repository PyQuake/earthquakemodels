...
```
#Summary
In this document we show two plots for the GAModel and the GABBOB - a simple GA applied to the BBOB benchmark funtions -  with tournize size (k) from 2 to 25, with the Uniform crossover operator (with 0.1 of independent probability of exchanging an gene).
...
Number of generations: 500 (used to be 100)
Population size: 800 (used to be 500)

#Comments

We are experimenting with 40 dimensions. Thats just because I thought that it was harder for a GA, as the earthquake modeling is. I got the idea from Sawyerr et al (Check next paragraph), although they use a much more complex GA (a hybrid one).
...
1. Maybe we should change to 20 dimensions or less. 
2. Maybe we should do more evaluations, in some cases it looks like the ga for the beanchmark functions did not converge at all.
3. In same cases, I do not think any of this will help. The functions are just too dificult for the GA. Even for hybrid GA, even for the best ones that got published in CEC - COCO conferences.
...
setwd("~/Documents/estudos/master-unb/earthquakemodels/result_exp_benchmark/UniformGaussian40D")
