...
def save2file(logbook, cx, mut, g, rank):
	f = open('parallel/'+region+'_'+str(year)+'_'+str(depth)+'_logboo8-2.txt',"a")
	data = str(logbook)+', '+str(g)+', '+str(rank)+', '+str(cx)+', '+str(mut)
	f.write(str(data))
	f.write('\n')
	f.close()


def gaModel(NGEN, n, modelOmega,year,region, depth):
	...
	for g in range(NGEN):
		...
		if (g%10) == 0:
			best_pop = tools.selBest(pop, 1)[0]
			data = comm.sendrecv(sendobj=best_pop,dest=dest,source=origin)
			#rotation
			target+=1
			origin = (rank - (target+1)) % size
			dest = (rank + ((target+1) + size)) % size

			pop[random.randint(0, len(pop)-1)] = data
			del best_pop
			del data

		record = stats.compile(pop)		#
		logbook.record(**record)
		save2file(record, CXPB, MUTPB, g, rank)


	if rank == 0:
		best_pop=tools.selBest(pop, 1)
		...
		print('\n\nvalor do log max',maximo, rank, '\n\n')
	else: 
		best_pop=tools.selBest(pop, 1)[0]
		req = comm.isend(best_pop, dest=0)
...
def migrator():
	...
	GSIZE = 100
	POPSIZE = 25
	gaModel(GSIZE, POPSIZE, observations,year+qntYears,region, depth)
